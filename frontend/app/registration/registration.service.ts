    ...
    private fetchUserInfo() {
        if (this.token != null) {
            ...
            this.http.get(this.oAuthUserUrl, { headers: headers })
                .map(res => res.json())
                .subscribe(info => {
                    let user: User = new User(info.name.givenName, info.name.familyName, info.gender, null, info.placesLived ? info.placesLived[0].value : null, "", "", new Image(info.id, "Profile Avatar", "jpg", info.image.url, "Profile Avatar"));
